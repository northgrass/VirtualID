...
object Correlation_New_01 {

  def main(args: Array[String]) {

    if (args.length != 3) {
      sys.error("Usage:<input><output><threshold>")
    }

    val Array(input, output, threshold) = args
    ...
    val oriRatings = sc.textFile(input).map(_.split(",")).filter(_.length > 3).map(fields => {
    ...
    }).filter(_._3 > threshold.toDouble).map(field => {
      //      println(field)
      field

    })

    //    groupBy(k => k._1).flatMap(x => (x._2.toList.sortWith((x, y) => x._3 > y._3).take(10000)))
    ...
    val tempSimilarities =
      vectorCalcsTotal.map(fields => {
        ...
        //               val cosSim = cosineSimilarity(dotProduct, scala.math.sqrt(ratingNormSq), scala.math.sqrt(rating2NormSq))
        val corr = newSimilarity(size, ratingSum, numRaters, numRaters2)

        val jaccard = jaccardSimilarity(size, numRaters, numRaters2)
        //        val corr = correlation(size, dotProduct, ratingSum, rating2Sum, ratingNormSq, rating2NormSq)
        (key._1, (key._2, jaccard, ratingSum, corr, size, cosSim, numRaters, numRaters2))
      })
  ...
  def newSimilarity(usersInCommon: Double, ratingSum: Double, totalUsers1: Double, totalUsers2: Double) = {
    val union = totalUsers1 + totalUsers2 - usersInCommon
    ratingSum / usersInCommon
  }
}