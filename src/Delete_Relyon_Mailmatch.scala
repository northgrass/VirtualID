...
object Delete_Relyon_Mailmatch {
...
//      val oriRatings1 = sc.textFile(input1).repartition(30).map(_.split(",")).filter(_.length == 2).map(fields => {
//
...
//      val oriRatings2=oriRatings1.map(x=>(x._2,x._1))
    ...
    val remainvid = sc.textFile(input2).filter(_.split(",").length == 2).map(x => {
      ...
      if(hsmap.contains(nick1 + "," + nick2type)){
        ...
        if(hsmap.get(nick2 + "," + nick1type).equals("Some(" + nick1id + ")")){
          outputString = x + ",,,2"
        }
