...
object Join4 {
  ...
  def main(args: Array[String]) {
    ...
    val denominatorFilter = sc.textFile(input1).map(_.split(",")).filter(_.length == 13).map(fields => {

      val nick1 = fields(1).trim()
      val info = fields(0).trim() + "," + fields(1).trim() + "," + fields(2).trim() + "," + fields(3).trim() + "," + fields(4).trim() + "," + fields(5).trim() + "," + fields(6).trim() + "," + fields(7).trim() + "," + fields(8).trim()+ "," + fields(9).trim() + "," + fields(10).trim() + "," + fields(11).trim() + "," + fields(12).replace("Some(", "").replace(")", "").trim()
