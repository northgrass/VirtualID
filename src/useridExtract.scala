...
object useridExtract {
  ...
  def main(args: Array[String]) = {
    ...
    var nick3_type = ""
    //    var serialNum = 0
    val timeAndID = sc.textFile(input).map(_.split("\\|")).filter(_.length > 65)
      .map(x => {
        //    val y :String = URLEncoder.encode(x,"UTF8")
        val length = x.length

        if (length == 65) {
          time = x(USERIP_INDEX_65)
          userip = x(USERIP_INDEX_65)
          host = x(HOST_65)
          uri = x(URI_INDEX_65)
          cookie = x(COOKIE_INDEX_65)
        } else if (length > 65) {
          time = x(LOGTIMESTAMP_INDEX_67)
          userip = x(USERIPv4_INDEX_67)
          pid_type = "useripV4"
          if (userip == "") {
            userip = x(USERIPv6_INDEX_67)
            pid_type = "useripV6"
          }
          uri = x(URI_INDEX_67)
          cookie = x(COOKIE_INDEX_67)
          host = x(HOST_67)
          useragent = x(USERAGENT_67)
        }
...
//                println("nick +type " + nick1 + nick1_type)
            ...
            if ((uri.contains("hrtx")) || (cookie.contains("hrtx"))) { // hrtx
              ...
              if (nick.contains("QQ_normal")) {
                if (nick.split("\\|").length > 1) {
                  //                println("nick=" + nick)
                  nick_type = "QQ_normal"
                  //                  println("hostdebug1=" + host)
                  nick = nick.split("\\|")(1)
                }
              }
            }
            if (nick == "") {
              //          if (nick.matches("^&&.*")) { // for debuge
              ...
              if (nick.contains(",")) {
                ...
                //                if ((nick.split(",").length > 1) && (nick.split(",")(1) != "")) {
                //                  nick2 = nick.split(",")(1)
                //                  if (nick2.contains("QQ_company")) {
                //                    if (nick2.split("\\|").length > 1) {
                //                      nick2_type = "QQ_company"
                //                      nick2 = nick2.split("\\|")(1)
                //                    }
                //                  }
                //                  //              println("nick2"+nick2)
        ...
        if (nick != "") { //single output
          ...
          //          if (nick3 != "") {
          //            if ((outputString != "")) {
          //              outputString = outputString + "\n" + time + "|" + userip + "|" + nick3 + "|" + host + "|" + uri + "|" + cookie
          //            } else if ((outputString == "")) {
          //              outputString = time + "|" + userip + "|" + nick3 + "|" + host + "|" + uri + "|" + cookie
          //            }
          //          }
        }
      ...
      //      .map(_.split("\\|")).filter(_.length > 1).map(x => {
      //
      ////        println("x length" + x.length)
      //
      //        var round = x.length / 7
      //        var i: Int = round
      //        var output = ""
      //        while (i != 0) {
      ////          println("i=" + i)
      //          val time = x(7 * (i.toInt - 1))
      ////          println("time" + time)
      //          val type1 = x(7 * (i.toInt - 1) + 1)
      ////          println("type1" + type1)
      //          val id_1 = x(7 * (i.toInt - 1) + 2)
      //          val type2 = x(7 * (i.toInt - 1) + 3)
      //          val id_2 = x(7 * (i.toInt - 1) + 4)
      //          val host = x(7 * (i.toInt - 1) + 5)
      //          val uri = x(7 * (i.toInt - 1) + 6)
      //          i = i - 1
      //          var tempout = ""
      //          if (output != "") {
      //            output = output + "\n" + time + "|" + type1 + "|" + id_1 + "|" + type2 + "|" + id_2 + "|" + host + "|" + uri
      //          } else {
      //            output = output + time + "|" + type1 + "|" + id_1 + "|" + type2 + "|" + id_2 + "|" + host + "|" + uri
      //          }
      //
      //        }
      ////        println(output)
    ...
    //    timeAndID.foreachPartition {
    //      //          timeAndID.repartition(3).foreachPartition {
    //      everyrecord =>
    //
    //        val driver = "org.postgresql.Driver";
    //        val url = "jdbc:postgresql://192.168.80.71/testDB";
    //        val user = "gpadmin";
    //        val password = "gpadmin";
    //
    //        try {
    ...
    //        var count = 0
    //
    //        val sql = "insert into " + output + " (timestamp,P_type,PID,S_type,SID,host,uri) values(?,?,?,?,?,?,?)"
    //        var pstmt: PreparedStatement = con.prepareStatement(sql)
    //
    //        everyrecord.foreach { y =>
    //          {
    //            count = count + 1
    //            var timeStamp: Timestamp = new Timestamp(Calendar.getInstance().getTimeInMillis())
    //
    //            if (time.matches("^(\\d){5,}$")) {
    //              timeStamp = TimeStamp2Date(time, "yyyy-MM-dd HH:mm:ss")
    //
    //              if (outputString != "") {
    //
    //                if (nick != "") {
    ...
    //                  pstmt.setString(2, pid_type);
    //                  pstmt.setString(3, userip)
    //                  pstmt.setString(4, nick_type)
    //                  pstmt.setString(5, nick)
    //                  pstmt.setString(6, host)
    //                  if (uri.length() > 2047) {
    //                    uri = uri.substring(0, 2040)
    //                  }
    ...
    //                  pstmt.setString(2, pid_type);
    //                  pstmt.setString(3, userip)
    //                  pstmt.setString(4, nick1_type)
    //                  pstmt.setString(5, nick1)
    //                  pstmt.setString(6, host)
    //                  if (uri.length() > 2047) {
    //                    uri = uri.substring(0, 2040)
    //                  }
    ...
    //                  pstmt.setString(2, pid_type);
    //                  pstmt.setString(3, userip)
    //                  pstmt.setString(4, nick2_type)
    //                  pstmt.setString(5, nick2)
    //                  pstmt.setString(6, host)
    //                  if (uri.length() > 2047) {
    //                    uri = uri.substring(0, 2040)
    //                  }
    ...
    //          }
    //        }
    //        //        myTable.flushCommits() //閸忔娊鏁悙锟?
    //        pstmt.executeBatch()
    //        pstmt.close()
    //
    //        //        con.commit() //閹笛嗩攽鐎瑰苯鎮楅敍灞惧閸斻劍褰佹禍銈勭皑閸旓拷
    //        //        con.setAutoCommit(true) //閸︺劍濡搁懛顏勫З閹绘劒姘﹂幍鎾崇磻
    //        con.close()
    //
    //    }

    //    timeAndID.saveAsTextFile(output)
